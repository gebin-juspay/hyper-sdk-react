# latam-juspay-payments-sdk-react
...
```sh
npm install latam-juspay-payments-sdk-react
```
...
Add the latamJuspayClientId ext property in root(top) `build.gradle`:
...
buildscript {
    ...
    ext {
        ....
        latamJuspayClientId = "<clientId shared by Juspay team>"
...
post_install do |installer|
 fuse_path = "./Pods/LatamJuspayPayments/Fuse.rb"
...
const { LatamJuspayPaymentsSdkReact } = NativeModules;
...
```ts
LatamJuspayPaymentsSdkReact.preFetch(JSON.stringify(preFetchPayload));
```
...
```ts
LatamJuspayPaymentsSdkReact.createHyperServices();
```
...
```ts
LatamJuspayPaymentsSdkReact.initiate(JSON.stringify(initiatePayload));
```
...
```ts
LatamJuspayPaymentsSdkReact.process(JSON.stringify(processPayload));
```
...
```ts
LatamJuspayPaymentsSdkReact.processWithActivity(JSON.stringify(processPayload));
```

### Step-4: Listen to events from HyperSDK

`Hyper SDK` Native Module will be emitting all the relevant events to JS via `RCTDeviceEventEmitter` and JavaScript modules can then register to receive events by invoking `addListener` on the `NativeEventEmitter` class in the `componentDidMount()` method with the event name `'LatamJuspayPaymentsEvent'` (You can use the `LatamJuspayPaymentsSdkReact.LatamJuspayPaymentsEvent` as well). The listener will return a `stringified JSON` response (`resp`).
...
```ts
 componentDidMount() {
   ...
   const eventEmitter = new NativeEventEmitter(NativeModules.LatamJuspayPaymentsSdkReact);
   this.eventListener = eventEmitter.addListener(LatamJuspayPaymentsSdkReact.LatamJuspayPaymentsEvent, (resp) => {
...
If the blocking asynchronous call `LatamJuspayPaymentsSdkReact.onBackPressed()` returns true, `Hyper SDK` will handle the back press, else merchant can handle it.

**Note**: `LatamJuspayPaymentsSdkReact.isNull()` (refer [here](#helper-is-null)) can also be called before calling `onBackPressed()` to ensure that the HyperServices object is not null.

```ts
 componentDidMount() {
   ...
   BackHandler.addEventListener('hardwareBackPress', () => {
     return !LatamJuspayPaymentsSdkReact.isNull() && LatamJuspayPaymentsSdkReact.onBackPressed();
   });
   ...
 }
...
```ts
LatamJuspayPaymentsSdkReact.terminate();
```
...
```ts
var isNull: boolean = LatamJuspayPaymentsSdkReact.isNull();
...
```ts
LatamJuspayPaymentsSdkReact.isInitialised().then((init: boolean) => {
...
This is an optional method to update the base view controller in case if any new view controller is presented over top view controller after the SDK initiation. This method should be called before making `LatamJuspayPaymentsSdkReact.process()` call.
...
1. LatamJuspayHeaderAttached
1. LatamJuspayHeader
1. LatamJuspayFooter
1. LatamJuspayFooterAttached

You can follow the below syntax to attach the component.
```ts
LatamJuspayPaymentsSdkReact.notifyAboutRegisterComponent(LatamJuspayPaymentsSdkReact.LatamJuspayHeaderAttached)
AppRegistry.registerComponent(LatamJuspayPaymentsSdkReact.LatamJuspayHeaderAttached, () => CustomComponent);
```
...
Therefore the component must fire `LatamJuspayPaymentsSdkReact.updateMerchantViewHeight(<section_name>, <height>);`

For example
```ts
LatamJuspayPaymentsSdkReact.updateMerchantViewHeight(LatamJuspayPaymentsSdkReact.LatamJuspayHeader, 200);
```
...
```ts
  onLayout={(event) => {
        ...
        LatamJuspayPaymentsSdkReact.updateMerchantViewHeight(LatamJuspayPaymentsSdkReact.LatamJuspayHeader, height);
      }}
...
latam-juspay-payments-sdk-react is distributed under [AGPL-3.0-only](https://github.com/juspay/latam-juspay-payments-sdk-react/src/main/LICENSE.md) license.
