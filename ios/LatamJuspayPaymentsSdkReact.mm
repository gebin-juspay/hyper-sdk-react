...
#import "LatamJuspayPaymentsSdkReact.h"
...
#import "LatamJuspayPaymentsServices.h"

__weak static LatamJuspayPaymentsServices *_hyperServicesReference;
...
- (UIView * _Nullable)merchantViewForViewType:(NSString * _Nonnull)viewType {
    ...
    if ([viewType isEqual:@"HEADER"] && [registeredComponents containsObject:@"LatamJuspayHeader"]) {
        moduleName = @"LatamJuspayHeader";
    } else if ([viewType isEqual:@"HEADER_ATTACHED"] && [registeredComponents containsObject:@"LatamJuspayHeaderAttached"]) {
        moduleName = @"LatamJuspayHeaderAttached";
    } else if ([viewType isEqual:@"FOOTER"] && [registeredComponents containsObject:@"LatamJuspayFooter"]) {
        moduleName = @"LatamJuspayFooter";
    } else if ([viewType isEqual:@"FOOTER_ATTACHED"] && [registeredComponents containsObject:@"LatamJuspayFooterAttached"]) {
        moduleName = @"LatamJuspayFooterAttached";
    }
...
@implementation LatamJuspayPaymentsSdkReact
RCT_EXPORT_MODULE()

NSString *HYPER_EVENT = @"LatamJuspayPaymentsEvent";
NSString *JUSPAY_HEADER = @"LatamJuspayHeader";
NSString *JUSPAY_FOOTER = @"LatamJuspayFooter";
NSString *JUSPAY_HEADER_ATTACHED = @"LatamJuspayHeaderAttached";
NSString *JUSPAY_FOOTER_ATTACHED = @"LatamJuspayFooterAttached";
...
- (void) createPaymentServices:(NSString*) clientId {
    if (self.hyperInstance == NULL) {
        self.hyperInstance = [[LatamJuspayPaymentsServices alloc] initWithClientId:clientId];
...
- (NSArray<NSString *> *)supportedEvents {
    return @[@"LatamJuspayPaymentsEvent"];
}
...
RCT_EXPORT_METHOD(preFetch:(NSString *)data) {
    if (data && data.length>0) {
        @try {
            NSDictionary *jsonData = [LatamJuspayPaymentsSdkReact stringToDictionary:data];
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                // [LatamJuspayPaymentsServices preFetch:jsonData];
            } else {
...
RCT_EXPORT_METHOD(createLatamJuspayPaymentsServices:(NSString*) clientId) {
    ...
    if (data && data.length>0) {
        @try {
            ...
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                ...
                __weak LatamJuspayPaymentsSdkReact *weakSelf = self;
                ...
                [_hyperInstance initiate:baseViewController payload:jsonData callback:^(NSDictionary<NSString *,id> * _Nullable data) {
                    [weakSelf sendEventWithName:@"LatamJuspayPaymentsEvent" body:[[self class] dictionaryToString:data]];
                }];
            } else {
                // Define proper error code and return proper error
                // [self sendEventWithName:@"LatamJuspayPaymentsEvent" body:[[self class] dictionaryToString:data]];
            }
        } @catch (NSException *exception) {
            // Define proper error code and return proper error
            // [self sendEventWithName:@"LatamJuspayPaymentsEvent" body:[[self class] dictionaryToString:data]];
...
{
    LatamJuspayPaymentsServices *hyperServicesInstance = _hyperServicesReference;
    if (payload && payload.length>0) {
        @try {
            NSDictionary *jsonData = [LatamJuspayPaymentsSdkReact stringToDictionary:payload];
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
